...
def mark_first_1000_records():
    """Mark the first 1000 records with 'x' field"""
    
    print("Fetching first 1000 document IDs...")
    
    # Get the first 1000 document IDs
    first_1000_docs = list(collection.find({}, {"_id": 1}).limit(1000))
    
    print(f"Found {len(first_1000_docs)} documents")
    
    # Extract just the IDs
    doc_ids = [doc["_id"] for doc in first_1000_docs]
    
    # Update all these documents to add 'x' field
    result = collection.update_many(
        {"_id": {"$in": doc_ids}},
        {"$set": {"x": True}}  # or use {"x": "x"} if you want string value
    )
...
if __name__ == "__main__":
    print("Starting MongoDB Update...")
    print("Database: Honda_cars")
    print("Collection: listings")
    print()
    
    mark_first_1000_records()
